...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        switch (interaction.options.getSubcommand()) {
            case 'search': {
                // Fetch fresh vending machine data before searching
                let mapMarkersResponse = await rustplus.getMapMarkersAsync();
                if (!mapMarkersResponse || !mapMarkersResponse.mapMarkers) {
                    const str = client.intlGet(interaction.guildId, 'couldNotFetchVendingData');
                    await client.interactionEditReply(interaction, DiscordEmbeds.getActionInfoEmbed(1, str));
                ...
                // DEBUG: Log vending machine sellOrders for this item
                console.log(`[DEBUG] /market search at ${new Date().toISOString()}`);
                for (const vendingMachine of rustplus.mapMarkers.vendingMachines) {
                    ...
                    for (const order of vendingMachine.sellOrders) {
                        if (order.itemId == itemId || order.currencyId == itemId) {
                            console.log(`[DEBUG] VM: ${vendingMachine.name || 'Unnamed'} @ ${vendingMachine.location?.string || 'unknown'} | SellOrder:`, order);
                        }
                    }
                }
