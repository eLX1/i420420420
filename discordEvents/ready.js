...
module.exports = {
    ...
    async execute(client) {
        ...
        if (Config.api.enabled) {
            client.apiServer = new ApiServer(client);
            client.apiServer.start();
        }

        // Call onReady functions for commands
        for (const [name, command] of client.commands) {
            if (command.onReady) {
                try {
                    await command.onReady(client);
                } catch (error) {
                    client.log(client.intlGet(null, 'errorCap'), `Error in command onReady for ${name}: ${error.message}`, 'error');
                }
            }
        }
    },
