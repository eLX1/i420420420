...
module.exports = {
    ...
    getStorageMonitorGroupListEmbed: function (groups) {
        return {
            title: 'Storage Monitor Groups',
            description: groups.length > 0 ? groups.map(g => `• **${g.name}**: ${g.description || 'No description'}`).join('\n') : 'No groups found.',
            color: 0x00bfff
        };
    },

    getStorageMonitorGroupEmbed: function (guildId, serverId, groupId) {
        const instance = Client.client.getInstance(guildId);
        const group = instance.serverList[serverId].storageMonitorGroups[groupId];
        const rustplus = Client.client.rustplusInstances[guildId];

        let description = `**Group Name:** ${group.name}`;
        if (group.description) {
            description += `\n**Description:** ${group.description}`;
        }
        description += `\n**Monitors:** ${group.monitors.length}`;

        if (!rustplus) {
            return module.exports.getEmbed({
                title: `Storage Monitor Group: ${group.name}`,
                color: Constants.COLOR_DEFAULT,
                description: `${description}\n${Client.client.intlGet(guildId, 'statusNotConnectedToServer')}`,
                thumbnail: `attachment://storage_monitor.png`,
                footer: { text: `${instance.serverList[serverId].title}` },
                timestamp: true
            });
        }
        ...
        for (const monitorId of group.monitors) {
            ...
            const monitor = instance.serverList[serverId].storageMonitors[monitorId];
            const rustplusMonitor = rustplus.storageMonitors[monitorId];
            
            if (rustplusMonitor) {
                const items = rustplusMonitor.items;
                const capacity = rustplusMonitor.capacity;
                const usedSlots = items.length;
                
                monitorDetails += `\n• **${monitor.name}** (${monitorId}): ${usedSlots}/${capacity} slots`;
                totalItems += usedSlots;
                totalCapacity += capacity;
            } else {
                monitorDetails += `\n• **${monitor.name}** (${monitorId}): Not connected`;
            }
        }

        description += `\n**Total Items:** ${totalItems}/${totalCapacity}`;
        description += monitorDetails;
    ...
    getStorageMonitorGroupItemSearchEmbed: function (group, item, quantity) {
        return {
            title: `Item Search: ${item}`,
            description: `Found in group **${group.name}**: **${quantity}**`,
            color: 0x00bfff
        };
    },
}