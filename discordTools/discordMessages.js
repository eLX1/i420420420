...
module.exports = {
    sendMessage: async function (guildId, content, messageId, channelId, interaction = null) {
        ...
        if (message !== undefined) {
            ...
            // If we have a messageId but couldn't find the message, it was likely deleted
            // Send a new message instead of trying to edit the missing one
    ...
    sendServerMessage: async function (guildId, serverId, state = null, interaction = null) {
        ...
        } else if (!interaction && !message && server.messageId) {
            // Clear the invalid message ID if the message couldn't be found
            instance.serverList[serverId].messageId = null;
            Client.client.setInstance(guildId, instance);
        }
    },
    ...
    sendTrackerMessage: async function (guildId, trackerId, interaction = null) {
        ...
        if (!interaction && message && message.id) {
            ...
            Client.client.setInstance(guildId, instance);
        } else if (!interaction && !message && tracker.messageId) {
            // Clear the invalid message ID if the message couldn't be found
            instance.trackers[trackerId].messageId = null;
            Client.client.setInstance(guildId, instance);
        }
    },
    ...
    sendStorageMonitorMessage: async function (guildId, serverId, entityId, interaction = null) {
        ...
        if (!interaction && message && message.id) {
            ...
            Client.client.setInstance(guildId, instance);
        } else if (!interaction && !message && entity.messageId) {
            // Clear the invalid message ID if the message couldn't be found
            instance.serverList[serverId].storageMonitors[entityId].messageId = null;
            Client.client.setInstance(guildId, instance);
        }
    },
    ...
    sendStorageMonitorGroupList: async function (channel, groups) {
        const embed = DiscordEmbeds.getStorageMonitorGroupListEmbed(groups);
        await channel.send({ embeds: [embed] });
    },

    sendStorageMonitorGroupDetails: async function (channel, group, monitors, items) {
        const embed = DiscordEmbeds.getStorageMonitorGroupEmbed(group, monitors, items);
        await channel.send({ embeds: [embed] });
    },

    sendStorageMonitorGroupItemSearch: async function (channel, group, item, quantity) {
        const embed = DiscordEmbeds.getStorageMonitorGroupItemSearchEmbed(group, item, quantity);
        await channel.send({ embeds: [embed] });
    },

    sendStorageMonitorGroupMessage: async function (guildId, serverId, groupId, interaction = null) {
        const instance = Client.client.getInstance(guildId);
        ...
        const content = {
            embeds: [DiscordEmbeds.getStorageMonitorGroupEmbed(guildId, serverId, groupId)],
            components: DiscordButtons.getStorageMonitorGroupButtons(groupId),
            files: [new Discord.AttachmentBuilder(
                Path.join(__dirname, '..', 'resources/images/electrics/storage_monitor.png'))]
        };
        ...
        try {
            ...
            if (!interaction && message && message.id) {
                instance.serverList[serverId].storageMonitorGroups[groupId].messageId = message.id;
                Client.client.setInstance(guildId, instance);
            }
        } catch (error) {
            Client.client.log('Error', `Failed to send storage monitor group message: ${error.message}`);
        }
    },
}