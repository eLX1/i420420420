...
module.exports = {
    ...
    getCreateGroupModal() {
        const modal = new ModalBuilder()
            .setCustomId('modal_create_group')
            .setTitle('Create Storage Monitor Group')
            .addComponents(
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('group_name').setLabel('Group Name').setStyle(TextInputStyle.Short).setRequired(true)
                ),
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('group_description').setLabel('Group Description').setStyle(TextInputStyle.Paragraph).setRequired(false)
                )
            );
        return modal;
    },

    getEditGroupModal(group) {
        const modal = new ModalBuilder()
            .setCustomId(`modal_edit_group_${group.id}`)
            .setTitle('Edit Storage Monitor Group')
            .addComponents(
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('group_name').setLabel('Group Name').setStyle(TextInputStyle.Short).setValue(group.name).setRequired(true)
                ),
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('group_description').setLabel('Group Description').setStyle(TextInputStyle.Paragraph).setValue(group.description || '').setRequired(false)
                )
            );
        return modal;
    },

    getAddMonitorModal(groupId) {
        const modal = new ModalBuilder()
            .setCustomId(`modal_add_monitor_${groupId}`)
            .setTitle('Add Monitor to Group')
            .addComponents(
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('monitor_id').setLabel('Monitor ID').setStyle(TextInputStyle.Short).setRequired(true)
                )
            );
        return modal;
    },

    getRemoveMonitorModal(groupId) {
        const modal = new ModalBuilder()
            .setCustomId(`modal_remove_monitor_${groupId}`)
            .setTitle('Remove Monitor from Group')
            .addComponents(
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('monitor_id').setLabel('Monitor ID').setStyle(TextInputStyle.Short).setRequired(true)
                )
