        ...
        if (guild) {
            ...
            if (channel) {
                try {
                    ...
                    // Only log the error once per message ID to avoid spam
                    if (!Client.client.missingMessageCache) {
                        Client.client.missingMessageCache = new Set();
                    }
                    
                    if (!Client.client.missingMessageCache.has(messageId)) {
                        Client.client.missingMessageCache.add(messageId);
                        Client.client.log(Client.client.intlGet(null, 'errorCap'),
                            Client.client.intlGet(null, 'couldNotFindMessage', { message: messageId }), 'error');
                        
                        // Remove from cache after 5 minutes to allow retry if message is restored
                        setTimeout(() => {
                            if (Client.client.missingMessageCache) {
                                Client.client.missingMessageCache.delete(messageId);
                            }
                        }, 5 * 60 * 1000);
                    }
                }
