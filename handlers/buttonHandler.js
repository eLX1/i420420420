...
module.exports = async (client, interaction) => {
    ...
    if (interaction.customId.startsWith('DiscordNotification')) {
        ...
        // Diagnostic logging
        client.log('Debug', `[GroupAddSwitch] ids: ${JSON.stringify(ids)}`);
        client.log('Debug', `[GroupAddSwitch] server: ${server ? 'exists' : 'missing'}`);
        if (server) {
            client.log('Debug', `[GroupAddSwitch] switchGroups: ${JSON.stringify(server.switchGroups)}`);
        }

        if (!server || (server && !server.switchGroups.hasOwnProperty(ids.groupId))) {
            client.log('Debug', `[GroupAddSwitch] Group does not exist. Deleting message and returning.`);
            await interaction.message.delete();
            return;
        }

        client.log('Debug', `[GroupAddSwitch] Showing modal for groupId: ${ids.groupId}`);
