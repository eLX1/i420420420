...
const StorageMonitorGroupHandler = require('../handlers/storageMonitorGroupHandler');
...
module.exports = {
    ...
    async execute(rustplus, client, message) {
        ...
        if (message.content && message.content.startsWith('!item ')) {
            const [cmd, groupName, ...itemParts] = message.content.split(' ');
            const item = itemParts.join(' ');
            await StorageMonitorGroupHandler.handleInGameItemSearch(message, groupName, item);
            return;
        }
    },
};
