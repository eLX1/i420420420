...
class RustPlus extends RustPlusLib {
    ...
    updateEvents(event, message) {
        ...
        if (![commandCargoEn, commandHeliEn, commandSmallEn, commandLargeEn, commandChinookEn, commandTravelingVendorEn, commandDeepSeaEn].includes(event)) return;
    ...
    async getEntityInfoAsync(id, timeout = 10000) {
        try {
            ...
            const response = await this.sendRequestAsync(request, timeout).catch((e) => {
                return e;
            });

            // Log request and response if there's an error
            if (response && response.hasOwnProperty('error')) {
                this.log(Client.client.intlGet(null, 'errorCap'), 
                    `Request: ${JSON.stringify(request)}. Response: ${JSON.stringify(response)}`, 'error');
            }
    ...
    async isResponseValid(response) {
        if (response === undefined) {
            ...
            // Enhanced logging: show full response and context
            const errorDetails = {
                error: response.error,
                fullResponse: JSON.stringify(response),
                // Try to extract entityId from response if available
                entityId: response.entityId || response.request?.entityId || 'unknown'
            };
            this.log(Client.client.intlGet(null, 'errorCap'), 
                `${Client.client.intlGet(null, 'responseContainError', { error: response.error })} ` +
                `Full response: ${JSON.stringify(response)}`, 'error');
            return false;
        }
